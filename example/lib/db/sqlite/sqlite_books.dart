...
class SqliteBooks {
  ...
  final table = MTable(
    ...
    fields: [
      ...
      MFieldVarchar(name: 'title', isNullable: false, validators: [
  ...
  Future<({int count, SqlDatabaseResult rows})> getAllBooks(
    String sort,
    String order, {
    ...
    var books = await table.execute(db, query);
    if (books.error) {
  ...
  Future<SqlDatabaseResult> addNewBook({
  ...
  Future<SqlDatabaseResult> updateBook({
    required String id,
    required String title,
    required String author,
    required String publishedDate,
