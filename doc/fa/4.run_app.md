...
در طول توسعه، از `lib/serve.dart` (تولیدشده توسط `finch create`) استفاده کنید. این فایل `main()` از `app.dart` را اجرا کرده و فایل‌های widget و زبان را برای تغییرات رصد می‌کند و بدون راه‌اندازی مجدد کامل سرور، source map های دارت را به‌صورت خودکار مجدداً تولید می‌کند:
...
این watcher با هر تغییر فایل، مبدل‌های `LanguageToDart` و `WidgetToDart` را فراخوانی می‌کند و سپس به نوار debugger متصل‌شده اطلاع می‌دهد تا صفحه را بارگذاری مجدد کند.
...
## مدیریت خطاهای ثبت‌شده

با ثبت یک callback در `app.onError()` می‌توانید هر بار که خطایی از طریق `Console` ثبت می‌شود (مثلاً `Console.e(...)`) مطلع شوید، برای مثال برای ارسال آن به یک سرویس مانیتورینگ خارجی:

```dart
app.onError((error, type) {
  // 'type' سطح لاگ است، مثلاً 'error'
