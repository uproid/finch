...
فینچ به صورت خودکار زبان را از مسیر URL تشخیص می‌دهد (`/fa/users`، `/en/users`).

مقداری که در نهایت به دست می‌آید همیشه در برابر `FinchConfigs.languages` اعتبارسنجی می‌شود؛ اگر جزو زبان‌های پیکربندی‌شده نباشد (مثلاً یک کوکی نامعتبر یا قدیمی)، فینچ به `'en'` بازمی‌گردد.
