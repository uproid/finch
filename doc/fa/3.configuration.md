...
| `languages` | `List<String>` | کلیدهای نقشه زبان پیش‌فرض | فهرست سفید کدهای locale معتبر؛ درخواست برای زبانی که در این فهرست نباشد به زبان پیش‌فرض بازمی‌گردد. به [Localization](11.localization_i18n.md) مراجعه کنید |
...
FinchDBConfig(
  ...
  maxConnections: 10,  // اندازه pool اتصال کتابخانه mongo_dart (Db.pool)
)
```
...
FinchMysqlConfig(
  ...
  maxConnections: 10,  // اندازه MySQLConnectionPool
)
```
