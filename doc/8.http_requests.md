...
`getCookie`/`addCookie` encrypt and decrypt the value by default (`safe` defaults to `true` on both) — pass `safe: false` explicitly for a plain, human-readable cookie value:

```dart
// Encrypted (default) — value is unreadable/untamperable in the browser
var token = rq.getCookie('auth_token', def: '');
rq.addCookie('auth_token', 'abc123');

// Plain — value is stored as-is, readable in the browser's dev tools
...
The encryption key is `FinchConfigs.cookiePassword`. `addCookie` also accepts `duration`, `expires`, `path`, `domain`, `secure`, `httpOnly`, and `sameSite` for full control over the `Set-Cookie` header. See [Cookies and Sessions](12.cookies_and_sessions.md) for the full reference.
