---
doc_meta:
    group: "Advanced Features"
    icon: "ph-bold ph-robot"
---
# MCP Server Guide

This guide explains how to expose a **Model Context Protocol (MCP)** endpoint from your Finch application. MCP is a JSON-RPC–based protocol designed for AI agents and language models to discover and invoke server-side capabilities — tools, resources, and prompts — in a standardised way.

Finch's `McpServerController` supports **two MCP protocol versions at once** — `2025-11-25` and `2026-07-28` — and picks which one to speak per request automatically (see [Protocol Version Negotiation](#protocol-version-negotiation) below). You register tools/resources/prompts a single time via `McpBuilder`; the controller handles routing correctly for whichever version the connecting client speaks.

Finch integrates with the [`mcp_models`](https://pub.dev/packages/mcp_models) package to provide:

- **`McpServerController`** — an abstract Finch controller that handles all MCP JSON-RPC routing automatically.
- **`McpBuilder`** — a declarative builder for registering tools, resources, prompts, and custom method handlers.
- **`mcp_models`** — a zero-dependency Dart library with full coverage of the MCP `2026-07-28` schema.

---

## Installation

Add the dependency to your `pubspec.yaml`:

```yaml
dependencies:
  finch: ^1.3.0        # MCP support was introduced in 1.3.0
```

`mcp_models` is re-exported by `package:finch/mcp.dart`, so no separate dependency entry is required unless you want to use it standalone.

---

## How It Works

When a client (an AI agent, IDE plugin, etc.) sends an MCP request, it sends a standard JSON-RPC payload via HTTP to your Finch endpoint. `McpServerController` decodes the payload, routes it to the correct built-in handler or to a handler you registered with `McpBuilder`, and streams the response back as [Server-Sent Events (SSE)](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events).

```
Client ──POST /mcp/books──► McpServerController.index()
                                   │
                                   ▼
                           _dispatch(method, id, payload)
                                   │
                    ┌──────────────┼──────────────────┐
                    ▼              ▼                   ▼
              tools/call    resources/read       prompts/get
                    │              │                   │
                    ▼              ▼                   ▼
             McpBuilder       McpBuilder          McpBuilder
           .toolHandler()  .resourceHandler()  .promptHandler()
```

---

## Creating an MCP Controller

Import the library and extend `McpServerController`:

```dart
import 'package:finch/mcp.dart';

class MyMcpController extends McpServerController {
  @override
  void configure(McpBuilder mcp) {
    // Register tools, resources, and prompts here.
  }
}
```

> `McpServerController` extends Finch's `Controller`, so you have access to the `rq` (request) object inside `configure()`. Because Finch controllers are instantiated per request, `rq` is always live when `configure` runs.

---

## Registering Capabilities with McpBuilder

### Tools

A **tool** is a callable function that an AI agent can invoke. Register one with `mcp.tool()`:

```dart
mcp.tool(
  name: 'add',
  description: 'Adds two integers and returns the sum.',
  inputSchema: ToolSchema(
    type: 'object',
    properties: {
      'a': Schema(type: 'integer', description: 'First operand', title: 'A'),
      'b': Schema(type: 'integer', description: 'Second operand', title: 'B'),
    },
    required: ['a', 'b'],
  ),
  outputSchema: ToolSchema(
    type: 'object',
    properties: {
      'result': Schema(type: 'integer', description: 'The sum', title: 'Result'),
    },
    required: ['result'],
  ),
  handler: (CallToolRequest req) async {
    final args = req.params.arguments ?? {};
    final sum = (args['a'] as int) + (args['b'] as int);
    return CallToolResult(
      content: [TextContent(text: '$sum', mimeType: 'text/plain')],
      structuredContent: {'result': sum},
    );
  },
);
```

| Parameter | Type | Description |
|---|---|---|
| `name` | `String` | Unique tool identifier |
| `description` | `String` | Human-readable description shown to AI agents |
| `inputSchema` | `ToolSchema` | JSON Schema describing the expected arguments |
| `outputSchema` | `ToolSchema?` | JSON Schema describing the structured output (optional) |
| `handler` | `Future<CallToolResult> Function(CallToolRequest)` | Async handler invoked on `tools/call` |

### Resources

A **resource** exposes a URI-addressable data source (a file, a database record, an API response, etc.):

```dart
mcp.resource(
  name: 'config',
  uri: 'file:///config.json',
  description: 'Application configuration file.',
  handler: (ReadResourceRequest req) async {
    final content = await File('config.json').readAsString();
    return ReadResourceResult(
      contents: [
        TextResourceContents(
          uri: req.params.uri,
          text: content,
          mimeType: 'application/json',
        ),
      ],
    );
  },
);
```

| Parameter | Type | Description |
|---|---|---|
| `name` | `String` | Unique resource name |
| `uri` | `String` | The URI that identifies this resource |
| `description` | `String?` | Optional description |
| `handler` | `Future<ReadResourceResult> Function(ReadResourceRequest)` | Handler invoked on `resources/read` |

> You can also use `rq.url('path')` inside `configure()` to build absolute URIs dynamically, since `configure` is called with the live request context.

### Resource Templates

For parameterised URIs (e.g. `file:///books/{id}`), use `mcp.resourceTemplate()`:

```dart
mcp.resourceTemplate(
  name: 'book',
  uriTemplate: 'db:///books/{id}',
  description: 'Fetches a single book by ID.',
);
```

### Prompts

A **prompt** is a reusable message template that an AI agent can retrieve:

```dart
mcp.prompt(
  name: 'greet',
  description: 'Returns a greeting message.',
  handler: (GetPromptRequest req) async {
    final name = req.params.arguments?['name'] ?? 'World';
    return GetPromptResult(
      messages: [
        PromptMessage(
          role: Role.assistant,
          content: TextContent(text: 'Hello, $name!'),
        ),
      ],
    );
  },
);
```

| Parameter | Type | Description |
|---|---|---|
| `name` | `String` | Unique prompt identifier |
| `description` | `String?` | Optional description |
| `handler` | `Future<GetPromptResult> Function(GetPromptRequest)` | Handler invoked on `prompts/get` |

### Custom Method Handlers

Override or extend any JSON-RPC method — including built-in ones — with `mcp.method()`:

```dart
mcp.method(
  'notifications/initialized',
  (Map<String, Object?> payload) async {
    // Custom logic on client initialisation notification.
    return JSONRPCNotification(method: 'notifications/initialized');
  },
);
```

Custom handlers registered via `mcp.method()` take priority over the built-in dispatcher.

---

## Protocol Version Negotiation

Which protocol version a request is dispatched as is decided per-request, checked in this order:

1. The `Mcp-Protocol-Version` HTTP header (sent by real MCP clients on every request after their initial handshake).
2. `params.protocolVersion` in the JSON-RPC payload (sent on the initial handshake call).
3. `params._meta['io.modelcontextprotocol/protocolVersion']` in the payload.
4. If none of the above is present, it defaults to `2025-11-25`.

Any value containing `"2026"` is treated as `2026-07-28`; everything else is treated as `2025-11-25`. This means a client that never identifies its version at all will always get the older `2025-11-25` dispatch table below.

## Built-in JSON-RPC Methods

`McpServerController` handles the following MCP methods automatically. The two protocol versions have **different built-in method sets** — most notably, `2026-07-28` replaces the handshake method `initialize` with `server/discover`, and drops `notifications/initialized`/`logging/setLevel` entirely:

| JSON-RPC Method | `2025-11-25` | `2026-07-28` | Action |
|---|---|---|---|
| `initialize` | ✓ | — | Returns server capabilities and info (`InitializeResult`) |
| `server/discover` | — | ✓ | Returns server capabilities, `supportedVersions`, and a cache TTL (`DiscoverResult`) — the `2026-07-28` equivalent of `initialize` |
| `tools/list` | ✓ | ✓ | Lists all registered tools |
| `tools/call` | ✓ | ✓ | Dispatches to the matching tool handler |
| `resources/list` | ✓ | ✓ | Lists all registered resources |
| `resources/read` | ✓ | ✓ | Dispatches to the matching resource handler |
| `resources/templates/list` | ✓ | ✓ | Lists all registered resource templates |
| `prompts/list` | ✓ | ✓ | Lists all registered prompts |
| `prompts/get` | ✓ | ✓ | Dispatches to the matching prompt handler |
| `notifications/initialized` | ✓ | — | Acknowledges client initialisation |
| `logging/setLevel` | ✓ | — | Accepts log-level requests (no-op by default) |

A custom handler registered with `mcp.method(name, handler)` is checked **before** this table for both versions, so it can override or add any method name — including `initialize`/`server/discover` themselves. Any other unrecognized method receives a standard `Method not found (-32601)` error response.

---

## Registering the Route

Add the MCP controller to your Finch route tree with `Methods.ALL` so both `GET` and `POST` requests are accepted:

```dart
import 'package:finch/finch_route.dart';
import 'controllers/my_mcp_controller.dart';

List<FinchRoute> getRoutes() {
  return [
    FinchRoute(
      key: 'mcp.my_server',
      path: 'mcp/my-server',
      methods: Methods.ALL,
      index: MyMcpController().index,
    ),
  ];
}
```

### With Authentication

Protect your MCP endpoint by providing an `AuthController`:

```dart
import 'controllers/mcp_auth_controller.dart';

FinchRoute(
  key: 'mcp.my_server',
  path: 'mcp/my-server',
  methods: Methods.ALL,
  index: MyMcpController().index,
  auth: McpAuthController(),
),
```

A typical `AuthController` for MCP checks a Bearer API key:

```dart
import 'package:finch/finch_route.dart';

class McpAuthController extends AuthController<String> {
  final List<String> _allowedKeys = ['your-secret-api-key'];

  @override
  Future<bool> auth() async => (await checkLogin()).success;

  @override
  Future<bool> authApi() async {
    final auth = rq.authorization;
    if (auth.type == AuthType.bearer) {
      return _allowedKeys.contains(auth.token);
    }
    return false;
  }

  @override
  Future<String> loginForm() async => rq.renderJson(
        data: {'error': 'Unauthorized'},
        status: 401,
      );
}
```

---

## Full Example

The following example mirrors the books controller in the Finch example application:

```dart
import 'package:finch/mcp.dart';

class McpServerBooksController extends McpServerController {
  @override
  void configure(McpBuilder mcp) {
    // ── Tool: list books ────────────────────────────────────────────────
    mcp.tool(
      name: 'get_all_books',
      description: 'Retrieves all books from the database.',
      inputSchema: ToolSchema(
        type: 'object',
        properties: {
          'limit': Schema(
            type: 'integer',
            description: 'Maximum number of books to retrieve.',
            defaultValue: 10,
            title: 'Limit',
          ),
        },
        required: ['limit'],
      ),
      handler: (req) async {
        final limit = req.params.arguments?['limit'] as int? ?? 10;
        // ... fetch from DB ...
        return CallToolResult(
          content: [TextContent(text: '[]', mimeType: 'application/json')],
          structuredContent: {'books': []},
        );
      },
    );

    // ── Tool: insert book ───────────────────────────────────────────────
    mcp.tool(
      name: 'insert_new_book',
      description: 'Inserts a new book into the database.',
      inputSchema: ToolSchema(
        type: 'object',
        properties: {
          'title': Schema(type: 'string', description: 'Book title.', title: 'Title'),
          'author': Schema(type: 'string', description: 'Author name.', title: 'Author'),
        },
        required: ['title', 'author'],
      ),
      handler: (req) async {
        final args = req.params.arguments ?? {};
        // ... insert into DB ...
        return CallToolResult(
          content: [],
          structuredContent: {'id': '42'},
        );
      },
    );

    // ── Resource: book list ─────────────────────────────────────────────
    mcp.resource(
      name: 'books',
      uri: rq.url('api/books'),
      description: 'Live JSON feed of all books.',
      handler: (req) async => ReadResourceResult(contents: []),
    );

    // ── Prompt: recommend ───────────────────────────────────────────────
    mcp.prompt(
      name: 'recommend_book',
      description: 'Ask the AI to recommend a book by genre.',
      handler: (req) async {
        final genre = req.params.arguments?['genre'] ?? 'fiction';
        return GetPromptResult(
          messages: [
            PromptMessage(
              role: Role.user,
              content: TextContent(
                text: 'Recommend a $genre book.',
              ),
            ),
          ],
        );
      },
    );
  }
}
```

Route registration:

```dart
FinchRoute(
  key: 'mcp.books',
  path: 'mcp/books',
  methods: Methods.ALL,
  index: McpServerBooksController().index,
  auth: McpAuthController(),
),
```

---

## mcp_models Package

`mcp_models` provides plain Dart classes for every type in the MCP `2026-07-28` schema. There is no code generation — every class ships with:

- A `toMap()` method for serialisation.
- A named factory `TypeName.toMCP(Map)` for deserialisation.

### Key Types

| Category | Types |
|---|---|
| **JSON-RPC** | `JSONRPCRequest`, `JSONRPCResultResponse`, `JSONRPCErrorResponse`, `JSONRPCNotification` |
| **Lifecycle** | `InitializeRequest`, `InitializeResult`, `InitializeResultResponse` |
| **Tools** | `Tool`, `ToolSchema`, `Schema`, `CallToolRequest`, `CallToolResult`, `CallToolResultResponse`, `ListToolsResult`, `ListToolsResultResponse` |
| **Resources** | `Resource`, `ResourceTemplate`, `ReadResourceRequest`, `ReadResourceResult`, `TextResourceContents`, `BlobResourceContents`, `ListResourcesResult`, `ListResourceTemplatesResult` |
| **Prompts** | `Prompt`, `PromptMessage`, `GetPromptRequest`, `GetPromptResult`, `ListPromptsResult` |
| **Content** | `TextContent`, `ImageContent`, `AudioContent`, `EmbeddedResource` |
| **Capabilities** | `ServerCapabilities`, `ClientCapabilities`, `Implementation` |
| **Errors** | `Error` (JSON-RPC error object) |
| **Misc** | `Result`, `EmptyResult`, `Role`, `LoggingLevel` |

### Serialisation Example

```dart
import 'package:mcp_models/mcp_models.dart';

// Build an initialize request.
final request = InitializeRequest(
  id: '1',
  params: InitializeRequestParams(
    protocolVersion: '2026-07-28',
    capabilities: ClientCapabilities({}),
    clientInfo: Implementation(name: 'my_client', version: '1.0.0'),
  ),
);

// Serialise to a Map (ready for JSON encoding).
final json = request.toMap();

// Deserialise back.
final restored = InitializeRequest.toMCP(json);
```

### ToolSchema and Schema

`ToolSchema` and `Schema` both describe JSON Schema objects used for tool input/output validation:

```dart
ToolSchema(
  type: 'object',
  properties: {
    'name': Schema(
      type: 'string',
      description: 'The name of the item.',
      defaultValue: '',
      title: 'Name',
    ),
    'count': Schema(
      type: 'integer',
      description: 'How many items.',
      defaultValue: 1,
      title: 'Count',
    ),
  },
  required: ['name'],
)
```

### MapMC and MapModel Base Classes

For types whose serialised form is the underlying map itself (rather than a wrapper object), `mcp_models` provides two base classes:

- **`MapMC<K, V>`** — extends `MCP`, serialises directly as its map.
- **`MapModel<K, V>`** — a plain map-like model without the `MCP` base.

`ServerCapabilities` and `ClientCapabilities` are built on `MapMC`.

---

## McpBuilder API Reference

| Method | Signature | Description |
|---|---|---|
| `tool()` | `void tool({name, description, inputSchema, outputSchema?, handler})` | Register a callable tool |
| `resource()` | `void resource({name, uri, description?, handler})` | Register a URI resource |
| `resourceTemplate()` | `void resourceTemplate({name, uriTemplate, description?})` | Register a URI template |
| `prompt()` | `void prompt({name, description?, handler})` | Register a prompt |
| `method()` | `void method(String method, Handler handler)` | Register a custom JSON-RPC method handler |
| `toolHandler()` | `ToolHandler? toolHandler(String name)` | Look up a tool handler by name |
| `resourceHandlerByUri()` | `ResourceHandler? resourceHandlerByUri(String uri)` | Look up a resource handler by URI |
| `promptHandler()` | `PromptHandler? promptHandler(String name)` | Look up a prompt handler by name |
| `methodHandler()` | `MethodHandler? methodHandler(String method)` | Look up a custom method handler |
| `buildToolsResult()` | `ListToolsResult buildToolsResult()` | Build a `tools/list` response |
| `buildResourcesResult()` | `ListResourcesResult buildResourcesResult()` | Build a `resources/list` response |
| `buildResourceTemplatesResult()` | `ListResourceTemplatesResult buildResourceTemplatesResult()` | Build a `resources/templates/list` response |
| `buildPromptsResult()` | `ListPromptsResult buildPromptsResult()` | Build a `prompts/list` response |

---

## Error Handling

Throw any `Exception` inside a tool handler — `McpServerController` wraps unhandled exceptions in a `JSONRPCErrorResponse` with code `-32600` and streams a `400` SSE response to the client. For structured error propagation, return a `CallToolResult` with `isError: true`:

```dart
handler: (req) async {
  final id = req.params.arguments?['id'];
  if (id == null) {
    return CallToolResult(
      isError: true,
      content: [TextContent(text: 'Missing required argument: id')],
    );
  }
  // ...
},
```

---

## See Also

- [mcp_models on pub.dev](https://pub.dev/packages/mcp_models)
- [MCP Specification 2026-07-28](https://spec.modelcontextprotocol.io/specification/2026-07-28/)
- [Finch Controllers](6.controllers.md)
- [Finch Auth Controller](7.auth_controller.md)
- [Finch Route](5.routing.md)
