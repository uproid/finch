...
| `register()` | Handles registration form submission, if your app supports self-registration | HTML response |
| `newUser()` | Creates a new user account (e.g. from an admin panel) | HTML response |
...
| `index()` | Inherited from `Controller` — the default handler if the route has no `index`/`controller` action of its own | Whatever this route should show |

If your app doesn't need registration, it's common to leave `register()`/`newUser()` throwing `UnimplementedError()`, as the example project does — see the example below.
...
`AuthType` also declares `digest`, `hawk`, `aws`, and `akamai` values for forward compatibility, but only `basic` and `bearer` are actually parsed today — any other scheme (or a missing/malformed header) resolves to `AuthType.none`.

## Permissions

`Permissions` is a small class of predefined permission-string constants you can use instead of hardcoding strings everywhere:
...
It only defines these two constants — for anything beyond `none`/`super-admin`, define your own permission strings (or extend `Permissions`) and check them the same way inside `checkPermission()`, as the example project's `AppAuthController.checkPermission()` does above.
